...
from module.Reasoning_reward import evaluate_responses
from module.utils import load_questions, send_openai_prompt
from module.latestoolbox import FactorizedTSAgent
...
def evaluate_few_shot_with_multiple_responses(ts_model, task, tokenizer, hf_model, max_attempts=10, output_file="results.json"):
    """
    ...
    with open(output_file, "w") as f:
        ...
        for idx, question_data in enumerate(task):
            ...
            if best_responses:
                ...
                reward = evaluate_responses(question_data['question'], best_responses=best_responses)
                print("reward is", reward)
                tem_action=step_length, prompt, temperature
                ts_model.update(context, reward, tem_action)

                print(f"Online Question {idx + 1}: Best Reward: {reward}")
        ...
        ts_model.save_parameters('TS_parameters_whole_MJ.pkl')
...
accuracy = evaluate_few_shot_with_multiple_responses(
    ...
    hf_model=hf_model,
    #max_attempts=3,
    output_file="./new_results/0209.json"
)
